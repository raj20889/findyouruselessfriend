...
export function MatchResultCard({ result, onReset, filePreviews }: MatchResultCardProps) {
  ...
  return (
    <Card className="w-full max-w-4xl text-center animate-in fade-in-50 zoom-in-90 duration-500 bg-card/70 backdrop-blur-lg shadow-2xl border-primary/20">
      <CardHeader>
      ...
      <CardContent className="flex flex-col items-center gap-6 p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
          ...
          <div className="w-full p-4 rounded-lg bg-muted/50 border border-border flex flex-col items-center justify-start">
            <h3 className="flex items-center justify-center text-2xl font-bold mb-3 gap-2"><Award className="text-accent"/> Useless Friend</h3>
            <div className="flex flex-col items-center gap-4 text-center">
              {uselessFriendImage ? (
                  <Image src={uselessFriendImage} alt="Useless Friend" width={120} height={120} className="rounded-full aspect-square object-cover border-4 border-accent shadow-lg" />
              ) : <UserX className="w-24 h-24 text-muted-foreground" />}
              <p className="text-muted-foreground text-lg flex-1 italic">
                The award goes to <Badge variant="secondary" className="text-base mx-1">Friend #{uselessFriend.index}</Badge> because: "{uselessFriend.reason}"
              </p>
            </div>
          </div>
